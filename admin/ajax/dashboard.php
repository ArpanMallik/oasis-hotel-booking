...
if (isset($_POST['booking_analytics'])) {
  ...
  $result = mysqli_fetch_assoc(mysqli_query($con, "SELECT
       ...
       SUM(CASE WHEN booking_status='pending'AND arrival= 1 THEN trans_amount END) AS `active_amount`,

       COUNT(CASE WHEN booking_status='cancelled'AND refund=1 THEN 1 END) AS `cancelled_bookings`,
       SUM(CASE WHEN booking_status='cancelled'AND arrival= 1 THEN trans_amount END) AS `cancelled_amount`
